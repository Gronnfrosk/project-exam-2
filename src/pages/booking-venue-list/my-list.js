...
const { upcomingBookings, previousBookings } = useBookingFilter(
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const params = userType === false ? "?_bookings=true&_sort=created&sortOrder=desc" : userType === true ? "?_venues=true&_sort=created&sortOrder=desc" : "";
  ...
  return <>
      <Helmet>
        <title>{profile.name} {userType === false ? "Bookings" : "Venues"} - Holidaze</title>
        <meta name="description" content="Profile inventory list" />
      </Helmet>
      <main className="list">
        {userType === false && profileResult.bookings ? (
          <>
            <BookingList upcomingBookings={upcomingBookings} previousBookings={previousBookings} profileResult={upcomingBookings}/>
          </>
