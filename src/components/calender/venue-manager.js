...
export default function CalenderManager(props) {
  ...
  const tileClassName = ({ date, view }) => {
    if (view === 'month') {
      ...
      bookings.forEach(booking => {
        const startDateStr = new Date(booking.dateFrom).toISOString().split('T')[0];
  ...
  return (
    <div className="venue-manager d-flex"> 
      ...
      />
     <div ref={listContainerRef} className="scrollable-container" style={{ height: '275px', overflowY: 'auto', marginTop: '20px', scrollBehavior: 'smooth' }}>
    <ListGroup style={{display: "flex", flexDirection: "column-reverse" }}>
      {bookings ? bookings.map((booking) => {
        ...
        variant={isActive ? "secondary" : "light"}
          ...
          style={{ fontWeight: booking.id === selectedBookingId ? 'bold' : 'normal', borderRadius: "0" }}
        >
          Booking ID: {booking.id} <br/> Guests: {booking.guests} <br/> From: {checkIn.toLocaleDateString("en-GB")} <br/> To: {checkOut.toLocaleDateString("en-GB")}
        </ListGroup.Item>
      }) : ""}
    </ListGroup>
  </div>
